...
.EXAMPLE
  # Marketing from company marketing config
  powershell -File ./scripts/deploy-swa-from-kv.ps1 `
    -ConfigPath ./infra/custom-domains.marketing.json `
    -DeployName marketing

.EXAMPLE
  # Explicit one-off (upload a pre-built folder)
  powershell -File ./scripts/deploy-swa-from-kv.ps1 `
    -VaultName ssd-global-kv-prod-ae `
...
.NOTES
  ...
  script runs `pnpm --filter @singleton-sd/marketing build` first.
#>
[CmdletBinding()]
param(
  [string]$ConfigPath = '',
...
if ($DeployName -or (-not $VaultName -and -not $SecretName -and -not $AppLocation)) {
  if (-not $ConfigPath) {
    $ConfigPath = Join-Path $repoRoot 'infra/custom-domains.marketing.json'
  }
...
if ($needsMarketingBuild -and -not (Test-Path $AppLocation)) {
  Write-Host "Marketing dist missing — building @singleton-sd/marketing..."
  Push-Location $repoRoot
  try {
    pnpm --filter @singleton-sd/marketing run build
...
if (-not (Test-Path $AppLocation)) {
  throw "AppLocation not found: $AppLocation (for marketing, run pnpm --filter @singleton-sd/marketing build first)"
}
