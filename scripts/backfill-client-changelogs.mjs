...
const PRODUCTS = {
  '@singleton-sd/marketing': ['apps/marketing'],
};
...
function main() {
  ...
  for (const plan of plans) {
    console.log(
      `${dryRun ? 'Would rewrite' : 'Rewriting'} ${plan.product}: ${plan.releases.length} releases`,
    );
    if (!dryRun) writeProductHistory(plan.product, plan.releases);
  }
}
