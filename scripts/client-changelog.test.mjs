...
test('prepends releases without dropping historical versions', () => {
  ...
  const output = JSON.parse(
    readFileSync(join(root, 'apps/marketing/src/data/changelog.json'), 'utf8'),
  );
  ...
  const markdown = readFileSync(join(root, 'apps/marketing/CHANGELOG.md'), 'utf8');
...
test('round-trips summaries and reasons through the canonical Markdown', () => {
  ...
  const markdown = formatProductChangelog('@singleton-sd/marketing', releases);
  assert.match(markdown, /generated from conventional commits by the release workflow/);
